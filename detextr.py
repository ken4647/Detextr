...
from process.run_fetgan import run_fetgan
from process.diffuser_opt import *
...
'''
    ...
    e.g. python detextr.py 1 1 永
    e.g. python detextr.py 1 2 6
    e.g. python detextr.py 1 3 口
'''
def get_options():
...
def main(opt):
    ...
    input_for_inpainting_dir = f'./image_repos/input_for_inpainting/{image_id}/{crop_id}'
    os.makedirs(input_for_inpainting_dir,exist_ok=True)
    ...
    mask_arr = make_mask_from_image(pasted_image, [point1, point2])   
    black_out_image = convert_to_image(black_out_image_arr)

    # Save images 
    cv2.imwrite(pasted_path := f'{input_for_inpainting_dir}/pasted_image.png', pasted_image)
    cv2.imwrite(edge_pasted_path := f'{input_for_inpainting_dir}/edge_pasted_image.png', edge_pasted_image)
    cv2.imwrite(black_out_path := f'{input_for_inpainting_dir}/balck_out_image.png', black_out_image)
    cv2.imwrite(original_path := f'{input_for_inpainting_dir}/original_image.png', original_image)
    cv2.imwrite(mask_arr_path := f'{input_for_inpainting_dir}/black_out.png', mask_arr)
