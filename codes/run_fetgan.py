...
def run_fetgan(src_pic_path=None,
               ref_pic_path=None,
               args=None
               ):
    ...
    opt['testsource'] = src_pic_path
    # opt['testsource'] = None #'./testimgs/TextEffects/FontEffects_sources/github.png' #None # single source image for test
    # opt['testsource_dir'] = './inputs/src_imgs/' # source image folder for test './testimgs/Fonts100/Fonts100_sources/', './testimgs/TextEffects/TextEffects_sources/'
    ...
    # # create a website
    web_dir = os.path.join(opt['testresults_dir'], opt['model_name'], args['image_id'], args['crop_id'])  # define the website directory
    webpage = html.HTML(web_dir, 'Test Experiment = %s, Epoch = %s' % (opt['model_name'], opt['epoch']))
